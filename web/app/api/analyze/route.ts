...
export async function POST(request: Request) {
  try {
    const { imageData, model = 'glm-4.6v', analysisType = 'photo', language = 'zh-CN' } = await request.json();
    ...
    const prompt = getAnalysisPrompt(analysisType, language);

    console.log(`Starting analysis with model: ${providerName}, type: ${analysisType}, lang: ${language}`);
